...
// Analyst operations
export function registerAnalyst(wallet: string, displayName: string, affiliateCode: string): Analyst {
  const db = getDb();
  const now = Date.now();
...
export function getAnalyst(wallet: string): Analyst | null {
  const db = getDb();
  const stmt = db.prepare(`
  ...
  return stmt.get(wallet) as Analyst | null;
}

export function getAnalystByCode(affiliateCode: string): Analyst | null {
  const db = getDb();
  const stmt = db.prepare(`
    SELECT wallet, display_name as displayName, affiliate_code as affiliateCode, created_at as createdAt
  ...
  return stmt.get(affiliateCode) as Analyst | null;
}
...
export function getAllAnalysts(): Analyst[] {
  ...
  return stmt.all() as Analyst[];
}

// Analysis operations
export function createAnalysis(analysis: MarketAnalysis): MarketAnalysis {
  const db = getDb();
  ...
  stmt.run(
    ...
    analysis.priceSol,
    analysis.createdAt,
    analysis.resolved ? 1 : 0,
    analysis.correct === null ? null : (analysis.correct ? 1 : 0)
  );
  
  return analysis;
}

export function getAnalysis(id: string): MarketAnalysis | null {
  const db = getDb();
  const stmt = db.prepare(`
...
export function getAnalysesByMarket(marketAddress: string): MarketAnalysis[] {
  const db = getDb();
  const stmt = db.prepare(`
    SELECT id, analyst_wallet as analystWallet, market_address as marketAddress, market_question as marketQuestion,
           thesis, recommended_side as recommendedSide, confidence, price_sol as priceSol, created_at as createdAt,
  ...
  return (stmt.all(marketAddress) as any[]).map(row => ({
    ...row,
    resolved: row.resolved === 1,
    correct: row.correct === null ? null : row.correct === 1
  }));
...
export function getAnalystAnalyses(wallet: string): MarketAnalysis[] {
  const db = getDb();
  const stmt = db.prepare(`
    SELECT id, analyst_wallet as analystWallet, market_address as marketAddress, market_question as marketQuestion,
           thesis, recommended_side as recommendedSide, confidence, price_sol as priceSol, created_at as createdAt,
  ...
  return (stmt.all(wallet) as any[]).map(row => ({
    ...row,
    resolved: row.resolved === 1,
    correct: row.correct === null ? null : row.correct === 1
  }));
...
export function updateAnalysisResolution(id: string, correct: boolean): void {
  const db = getDb();
  const stmt = db.prepare(`
...
export function getPurchasesByBuyer(wallet: string): any[] {
  const db = getDb();
  const stmt = db.prepare(`
...
// Stats calculation
export function getAnalystStats(wallet: string): AnalystStats {
  const db = getDb();
